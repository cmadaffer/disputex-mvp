...
export default function Home() {
  ...
  const CALENDLY_URL = 'https://calendly.com/YOUR-SLOT/intro' // replace or leave
  const TALLY_URL = 'https://tally.so/r/YOUR-FORM'             // replace or leave
  const SUPPORT_EMAIL = 'hello@disputex.ai'                    // replace or leave
  // --------------------------------

  // ROI calculator state
  const [gmv, setGmv] = useState(100000) // monthly sales volume USD
  const [disputeRate, setDisputeRate] = useState(1.5) // % orders disputed
  const [winRate, setWinRate] = useState(55) // % wins
  const [avgTicket, setAvgTicket] = useState(120) // $ per dispute
  const [feePct, setFeePct] = useState(15) // success fee during pilot
  ...
  // Waitlist form state (uses your /api/waitlist)
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Issuer-tuned CE 3.0 evidence packs. We assemble the right proof and submit on time, every time. Pay only when you win during the pilot."
        />
        <meta property="og:title" content="Disputex — Recover Chargebacks Automatically" />
        <meta
          property="og:description"
          content="Issuer-tuned representments, CE 3.0 evidence, automated deadlines. Pay only when you win."
        />
      ...
      <main>
        ...
        <header className="container">
          <div className="nav">
            ...
            <nav className="navlinks">
              ...
              <a className="btn" href={CALENDLY_URL} target="_blank" rel="noreferrer">Book a 12-min demo</a>
            </nav>
          </div>
        </header>
        ...
        <section className="hero container">
          <div className="heroContent">
            <h1>
              Recover chargebacks <span className="accent">automatically</span>.
            </h1>
            ...
            {/* Optional external CTAs */}
            <div className="ctaRow">
              <a className="btn xl ghost" href={CALENDLY_URL} target="_blank" rel="noreferrer">See a 12-min demo</a>
              <a className="btn xl ghost" href={TALLY_URL} target="_blank" rel="noreferrer">Use intake form</a>
            </div>
        ...
        <section id="features" className="features container">
          <Feature title="Issuer-tuned letters" text="Arguments and citations aligned to card-network guidance and your processor’s preferences." />
          ...
          <Feature title="What won & why" text="Post-decision analytics at reason-code level so you can prevent future losses." />
        </section>
        ...
        <section id="how" className="how container">
          <h2>How it works</h2>
          <div className="steps">
            <Step n={1} title="Connect & import" text="Securely share disputes and basic order data. No engineering required." />
            <Step n={2} title="Assemble evidence" text="We generate the issuer-ready narrative and attach CE 3.0 proof." />
            <Step n={3} title="Submit & track" text="We submit via your processor portals and track outcomes & deadlines." />
            <Step n={4} title="Learn & prevent" text="See what wins, fix what loses, and shrink future disputes." />
          </div>
        </section>
        ...
        <section className="roi container" aria-labelledby="roiTitle">
          <div className="roiCard">
            ...
            <div className="ctaRow center">
              <button onClick={() => document.querySelector('.waitlist input')?.focus()} className="btn xl">Start 30-day pilot</button>
        ...
        <section id="pricing" className="pricing container">
          <div className="priceCard">
            ...
            <a className="btn xl" href={CALENDLY_URL} target="_blank" rel="noreferrer">Book a 12-min demo</a>
            <span className="fine">Enterprise or agency? <a href={`mailto:${SUPPORT_EMAIL}`}>Contact sales</a></span>
          </div>
        </section>
        ...
        <footer className="footer">
          <div className="container footGrid">
            ...
            <div className="footLinks">
              ...
              <a href="#how">How it works</a>
              <a href="#pricing">Pricing</a>
              <a href={TALLY_URL} target="_blank" rel="noreferrer">Start pilot</a>
            </div>
            <div className="fine">
      ...
      <style jsx global>{`
        :root {
          ...
          --accent2: #60a5fa;
          --success: #34d399;
          --grad1: radial-gradient(800px 400px at 20% -10%, rgba(125,211,252,0.25), transparent 60%);
        ...
        * { box-sizing: border-box; }
        ...
        .ok { color: #8ff0b0; margin: .3rem 0 0; }
        .err { color: #ff8890; margin: .3rem 0 0; }
        ...
        .logos { text-align: center; padding: 1.8rem 0 0; }
        .logoRow { display: grid; grid-template-columns: repeat(5, 1fr); gap: 1rem; align-items: center; opacity: .6; }
        .logo { width: 100%; height: 40px; fill: #9fb3d2; }
        .hint { display: block; color: var(--muted); font-size: .9rem; margin-bottom: .8rem; }
        ...
        .how { padding: 2.5rem 0 0; }
        .how h2, .roi h2, .pricing h2, .faq h2 { font-size: clamp(1.6rem, 3.2vw, 2rem); margin: 0 0 1rem; }
        ...
        .roi { padding: 2.8rem 0 0; }
        .roiCard { background: linear-gradient(180deg, rgba(255,255,255,0.08), rgba(255,255,255,0.03)); border: 1px solid rgba(255,255,255,0.1); border-radius: 18px; padding: 1.2rem; }
        ...
        .k { display: block; font-size: 1.2rem; font-weight: 800; }
        .kLabel { color: var(--muted); font-size: .85rem; }
        .ctaRow.center { justify-content: center; gap: 1rem; align-items: center; }

        .pricing { padding: 2.6rem 0 0; }
        .priceCard { text-align: center; background: linear-gradient(180deg, rgba(255,255,255,0.08), rgba(255,255,255,0.03)); border: 1px solid rgba(255,255,255,0.1); border-radius: 18px; padding: 2rem 1.2rem; }
        .bullets { list-style: none; padding: 0; margin: 0 0 1rem; display: grid; gap: .5rem; }
        .bullets li { color: var(--muted); }

        .faq { padding: 2.4rem 0 3.6rem; }
        .faq details { background: var(--panel); border: 1px solid var(--panel-strong); border-radius: 12px; padding: .9rem 1rem; margin-bottom: .8rem; }
...
function Step({ n, title, text }) {
  return <div className="step"><div className="n">{n}</div><h4>{title}</h4><p>{text}</p></div>
}
...
function MetricsCard() {
  return (
    <div className="metrics">
      <div className="row">
        <Metric k={'24h'} label="Turnaround" />
        <Metric k={'CE 3.0'} label="Evidence-ready" />
        <Metric k={'$0'} label="If no recovery" />
      </div>
      <div className="barWrap" aria-hidden><div className="bar" /></div>
      <style jsx>{`
        .metrics { display: grid; gap: .9rem; }
        .row { display: grid; grid-template-columns: repeat(3, 1fr); gap: .6rem; }
        .barWrap { height: 6px; background: rgba(255,255,255,0.08); border-radius: 999px; overflow: hidden; }
        .bar { height: 100%; width: 65%; background: linear-gradient(90deg, var(--accent), var(--accent2)); border-radius: 999px; box-shadow: 0 0 20px rgba(125,211,252,0.5); animation: load 2.4s ease infinite alternate; }
        @keyframes load { from { width: 52%; } to { width: 88%; } }
        @media (max-width: 980px) { .row { grid-template-columns: 1fr; } }
      `}</style>
    </div>
  )
}
function Metric({ k, label }) {
  return (
    ...
    <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
      <defs>
        <linearGradient id="g" x1="0" y1="0" x2="26" y2="26" gradientUnits="userSpaceOnUse">
          <stop stopColor="#7dd3fc" /><stop offset="1" stopColor="#60a5fa" />
        </linearGradient>
      </defs>
      <rect x="1" y="1" width="24" height="24" rx="6" stroke="url(#g)" strokeWidth="2"/>
      <path d="M7 16c2.2-4.4 6.1-7.2 12-8-3.9 2.1-6.3 4.7-7.2 7.8 1.4-.6 3-.9 4.9-.9-2.7 1.6-5.2 2.5-7.7 2.6-1.9.1-3.1-.5-2-.5z" fill="url(#g)"/>
    </svg>
  )
}
